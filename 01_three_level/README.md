...
The borrowed atomic constants — the natural linewidth Γ, the hyperfine splittings, the centroids — are ⁸⁷Rb
[*D Line Data* (Steck)](../references/README.md); the 1064 nm polarizabilities are Chen–Raithel (2015).
Everything else in the table (U₀, ν_z, η) follows from them.

Two 1064 nm beams, **1 W each, counter-propagating**, make the lattice. The AC light shift of a level of
polarizability α in intensity I is the optical-dipole potential (Grimm, Weidemüller & Ovchinnikov 2000)
...
> (angular-momentum algebra, Edmonds 1957) kills the entire F′=2 hyperfine tensor. So |F′2,0⟩ sits at **+38 MHz independent of polarization geometry** —
...
by contrast, are insensitive only near B=0). An ⁸⁷Rb field-insensitive clock pair, with its differential light
shift cancelled, was demonstrated by Chicireanu *et al.* (2011). A residual **second-order (quadratic) Zeeman** differential remains;
...
heating rate A₊ — with steady state n̄_z = A₊ / (A₋ − A₊) (the rate-equation theory of Morigi 2003). With the cooling sideband on the bright peak, the
...
More detuning ⇒ lower floor, until photon recoil (~η² per scatter; the recoil limit of Wineland & Itano 1979)
takes over. That one formula — the EIT-cooling result of Morigi, Eschner & Keitel (2000) — is the heart of the scheme.
...
*Left: starting hot (n̄₀ ≈ 2.8), the axial motion cools to the floor (n̄_z ≈ 0.0020) with a 1/e time ≈ 70 µs.
Right: at steady state essentially all the population is in the motional ground state, P(n=0) ≈ 0.998. From the
same both-leg + emission recoil model as `cooling.py`, drawn by [`plots.py`](src/plots.py).*
...
| `plots.py` | the four figures (`eit_spectrum`, `cooling_curve`, `stark_manifold`, `lambda_scheme`) |
...
- **Morigi, Eschner & Keitel (2000)**, PRL **85**, 4458 — the EIT-cooling mechanism and the (Γ/4Δ)² floor.
- **Morigi (2003)**, PRA **67**, 033402 — the detailed rate-equation theory (cooling rates A±, the floor).
- **Wineland & Itano (1979)**, PRA **20**, 1521 — the recoil limit of laser cooling.
- **Grimm, Weidemüller & Ovchinnikov (2000)**, Adv. At. Mol. Opt. Phys. **42**, 95 — the optical-dipole potential U = −αI/(2ε₀c).
- **Chen & Raithel (2015)**, PRA **92**, 060501(R) — the 1064 nm scalar/tensor polarizabilities.
- **Edmonds (1957)**, *Angular Momentum in Quantum Mechanics* — the Wigner-6j / Clebsch–Gordan algebra.
- **Steck**, *Rubidium 87 D Line Data* — the ⁸⁷Rb linewidth, hyperfine splittings, and centroids.
- **Chicireanu *et al.* (2011)**, PRL **106**, 063002 — an ⁸⁷Rb field-insensitive clock pair with DLS cancellation.
