...
stark.py keeps the per-(F',m') tensor map numpy-only (hardcoded _FPREF) so it stays a clean,
...
for Fp in (0, 1, 2, 3):
    ...
    hard = stark._FPREF[Fp]
    match = abs(ratio - hard) < 1e-4
    ok = ok and match
    print(f"  F'={Fp}:  ratio = {ratio:+.5f}   stark._FPREF = {hard:+.5f}   {'OK' if match else 'MISMATCH'}")
