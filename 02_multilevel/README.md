...
dedicated F′1 repumpers — see [`../upgrades/`](../upgrades/README.md).
...
| `level_scheme.py` | the 24-level scheme figures on the 1064-shifted manifold — the baseline (no master) here, **and** the master-upgrade variant (written into `../upgrades/`); colour = comb line, solid/dashed = forward/retro, each beam's label is the Stark decomposition `WW(−s−t−g=ZZ)` (bare detuning → in-trap) |
