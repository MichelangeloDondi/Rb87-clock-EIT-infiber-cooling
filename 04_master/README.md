...
the leak to small Δ (the F′1 leak is [chapter 03](../03_dark_vertex/README.md)). One upgrade can break that tension:
...
the "curiosities" at the end of this chapter, not the realistic path.
...
resonant on |F′1,0⟩ too → scatters → 5/6 to F=1), which the master cannot fix — chapter 03 computes it (≈ 0.055).
...
excited scalar − excited tensor − ground scalar = in-trap detuning). Generated by [`../02_multilevel/level_scheme.py`](../02_multilevel/src/level_scheme.py).*
...
Recorded for completeness only — **curiosities, not the realistic path.**

**Dual-end double injection.** Inject from **both** fibre ends instead of retro-reflecting from one: control σ⁻
from one end, the carrier-suppressed EOM comb σ⁺ from the other — dropping the retro mirror and the tag AOM
entirely. With the same dedicated F′1 repumper, this removes the rejected comb tones near F′2 and the ~30 %
retro-efficiency penalty.

![dual-end double-injection bench](images/bench_dual_end.png)
...
topology), so it is **leak-limited at ≈ 0.055**, the same as the master. The cleaner delivery buys essentially
