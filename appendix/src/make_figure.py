...
axA.text(0.45, BASE * 1.22, "no canceller", color="0.4", fontsize=11, ha="right")
...
axA.annotate("proxy nulls the leak here\n(perfect dark state, scatter$\\to0$)", xy=(g_null, 2e-7),
             xytext=(g_null - 0.3, 3e-6), fontsize=11, color="#1a7a4a", ha="right",
             arrowprops=dict(arrowstyle="->", color="#1a7a4a", lw=1.0))
axA.annotate("the real tone only\nADDS scatter", xy=(-3.4, BASE + 3.25e-5 * 3.4 ** 2),
             xytext=(-4.4, 1.6e-3), fontsize=11, color="#c0392b", ha="right",
             arrowprops=dict(arrowstyle="->", color="#c0392b", lw=1.0))
axA.set_ylim(1e-7, 6e-3)
axA.set_xlim(-8.3, 0.5)
axA.tick_params(labelsize=12)
axA.set_xlabel(r"canceller amplitude on $|1,-1\rangle\!\to\!|F'1,0\rangle$,  $g_X$", fontsize=13)
axA.set_ylabel("total steady-state scatter  (∝ leak)", fontsize=13)
axA.set_title("Can a tone cancel the leak? The Floquet verdict\n"
              "principle: yes.  physical tone: no — the $\\Delta{+}157$ beat defeats it", fontsize=15)
axA.legend(fontsize=11, loc="lower left")
...
axB.annotate("coldest the D2 scheme delivers\n$\\bar n_z\\approx%.3f$ at $\\Delta\\approx%d$" % (fon[imin], Ds[imin]),
             xy=(Ds[imin], fon[imin]), xytext=(Ds[imin] + 6, fon[imin] + 0.020), fontsize=11, color="#c0392b",
             arrowprops=dict(arrowstyle="->", color="#c0392b", lw=1.1))
axB.text(37, 0.046, "leak penalty grows\nwith $\\Delta$  (∝ $\\Delta/(\\Delta{+}157)^2$)",
         fontsize=11, color="#a0610a", style="italic", ha="center")
axB.set_xlabel(r"control detuning  $\Delta$  (2$\pi$·MHz)", fontsize=13)
axB.set_ylabel(r"axial cooling floor  $\bar n_z$", fontsize=13)
axB.set_title("The leak-aware operating point\nsmaller $\\Delta$ is colder once the leak dominates", fontsize=15)
axB.set_xlim(13.5, 57)
axB.set_ylim(0, 0.125)
axB.tick_params(labelsize=12)
axB.legend(fontsize=11, loc="upper left")
axB.grid(alpha=0.25)

fig.suptitle("The leak frontier — cancellation fails; the D2 scheme bottoms out near 0.055", fontsize=16, y=1.00)
fig.tight_layout()
fig.savefig(os.path.join(IMAGES, "cancellation_verdict.png"), dpi=150, bbox_inches="tight")
