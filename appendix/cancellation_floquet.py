...
THE TWO CANCELLERS (both add a |1>->|3> coupling of amplitude g_X):
  * STATIC (in-frame): a hypothetical tone at the probe's OWN frame frequency, so its |3> coupling is
    time-independent. At g_X = Op*(R_c - R_p) it makes |D2> dark on 3 as well -> a PERFECT dark state ->
...
def scatter(g_X=0.0, resonant=True):
    """Total steady-state excited population (= total scatter / Gamma) with a canceller of amplitude g_X
    ...
    if not resonant:
        H = _H0 + g_X*_op(3, 1) + g_X*_op(1, 3)
        rho = qt.steadystate(H, _cops)
        return float(qt.expect(_Pe1, rho) + qt.expect(_Pe2, rho))
    H = [_H0, [g_X*_op(3, 1), lambda t: np.exp(-1j*_W*t)],
              [g_X*_op(1, 3), lambda t: np.exp(1j*_W*t)]]
    tl = np.linspace(0, 14.0, 7000)
    r = qt.mesolve(H, qt.steadystate(_H0, _cops), tl, c_ops=_cops, e_ops=[_Pe1, _Pe2])
    n = int((2*np.pi/_W) / (tl[1]-tl[0])) * 4        # average over the last ~4 beat periods
...
if __name__ == "__main__":
    ...
    print("static-null amplitude g_X = Op(R_c-R_p) = %.2f\n" % g_null)
    print("  g_X            STATIC (in-frame proxy)      RESONANT (the real tone)")
    print("  " + "-" * 68)
    for f in (0.5, 1.0, 1.5):
        gX = f * g_null
        print("  %+6.2f         total = %.3e             total = %.3e"
              % (gX, scatter(gX, False), scatter(gX, True)))
    print("  " + "-" * 68)
    print("VERDICT:")
    print("  static proxy at g_X=%.2f  ->  scatter ~ 0  (perfect dark state; the principle)." % g_null)
