...
axL.text(np.sqrt(IDEAL * MASTER), y.max() + 0.5,
         ...
         ha="center", va="bottom", fontsize=10.5, color="#a0610a", style="italic")
for yi, (label, val, col, mk, kind) in zip(y, rungs):
    ...
    axL.text(val * 1.18, yi, r"$\bar n_z\!\approx\!%.3f$%s" % (val, tag), va="center", ha="left",
             fontsize=11, color=col, fontweight=("bold" if kind == "computed" else "normal"))
    axL.text(RECOIL * 0.74, yi, label, va="center", ha="right", fontsize=10.5)
axL.axvline(RECOIL, color="0.5", ls=":", lw=1.2)
axL.set_xscale("log"); axL.set_xlim(RECOIL * 0.6, 0.20); axL.set_ylim(-0.45, y.max() + 1.15)
axL.set_yticks([])
axL.set_xlabel(r"axial cooling floor  $\bar n_z$   (lower = colder)", fontsize=13)
axL.set_title("The master floor, with the F'1 leak folded in", fontsize=14)
...
axR.text(1.0, fl.max(), " physical\n (f = 1)", fontsize=10, color="0.4", va="top")
axR.axvline(R_c / R_p, color="#c0392b", ls=":", lw=1.1)
axR.annotate("residual nulled\n$f=R_c/R_p=%.2f$" % (R_c / R_p), xy=(R_c / R_p, 0.043),
             xytext=(R_c / R_p + 0.45, IDEAL + 0.022), fontsize=10, color="#c0392b",
             arrowprops=dict(arrowstyle="->", color="#c0392b", lw=1.0))
axR.scatter([1.0], [0.082], s=120, color="#2980b9", edgecolor="#c0392b", linewidth=2.0, zorder=4)
axR.set_xlabel(r"scale on the probe's $|1,-1\rangle\!\to\!|F'1,0\rangle$ coupling,  $f$", fontsize=13)
axR.set_ylabel(r"axial cooling floor  $\bar n_z$", fontsize=13)
...
fig.suptitle(r"Chapter 03 — the second dark vertex: $|D_2\rangle$ is not dark on $|F'1,0\rangle$  "
             r"($R_c=%.2f,\ R_p=%+.2f$)" % (R_c, R_p), fontsize=15, y=1.02)
fig.tight_layout()
fig.savefig(os.path.join(IMAGES, "dark_vertex_floor.png"), dpi=150, bbox_inches="tight")
