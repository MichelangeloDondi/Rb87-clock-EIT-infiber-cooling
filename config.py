"""
config.py -- every physical number in one place. Two scripts read it:
  stark.py    the optical chain -> trap depth + the 5P Stark shifts   (closed form)
  cooling.py  the on-axis EIT cooling floor                            (one master equation)

Frequencies are ANGULAR, in 2*pi*MHz (a "6.07" means 2*pi*6.07 MHz).
Polarizabilities are in atomic units. The physics is in README.md.
"""
import numpy as np
...
# --- operating point (finalised single-ended tagged-retro point) ---
